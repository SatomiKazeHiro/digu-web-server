...
const db = new Database("./db/sources.db");
...
module.exports = class SourcesDB {
  /**
   * 获取数据库中的所有“域”名
  ...
  static getCategories(area, allCategoryInfo = true) {
    const readCategories = db.prepare(
  ...
  static getCategoryMsg(area, category) {
    const readCategoryMsg = db.prepare(
