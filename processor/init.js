...
let { scanFolder, scanItem } = require("./module");

/**
 * 资源目录初始化，深入遍历
 */
module.exports = init = () => {
  const ioLog = process.__tools.ioLog;
            ...
            if (itemArr.length > 0) {
              ...
              itemArr.forEach((item) => {
                ...
                if (SqlTool.findItem(itemObj.id)) {
                  ...
                  // 根据返回的标签判断是否需要更新数据
                  if (itemObj.up) {
                    SqlTool.update("item_msg", {
                      id: itemObj.id,
                      cover: itemObj.cover,
                      title: itemObj.title,
                      intro: itemObj.intro,
                      custom_cover: itemObj.custom_cover,
                      type: itemObj.type,
